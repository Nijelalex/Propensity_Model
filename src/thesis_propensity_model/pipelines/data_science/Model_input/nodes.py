...
def split_data(input_df: pd.DataFrame, config: Dict) -> pd.DataFrame:
    ...
    stratified_shuffle = StratifiedShuffleSplit(n_splits=1, test_size=0.3, random_state=1)
...
def standard_scaler(train_df: pd.DataFrame, test_df: pd.DataFrame, config: Dict) -> pd.DataFrame:
    """Splits the data into train-test.
    Args:
        df: Train_Df.
        df1: Test_Df.
